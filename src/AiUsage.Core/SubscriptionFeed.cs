...
public sealed class SubscriptionFeed
{
    sealed class Slot(string id, Func<CancellationToken, Task<UsageEntry>> fetch, TimeSpan interval)
    {
        ...
        public UsageEntry Current = new(id, Status: "연결 확인 중…");
    ...
    public UsageSnapshot Read(IReadOnlySet<string> enabled)
    {
        ...
        return stored with {
            ...
            Notice = live.Length == 0 ? null : $"연결 {ready}/{live.Length} · 자동 업데이트"
        };
    }
    ...
    async Task RefreshOneAsync(string id, Slot slot, bool force, CancellationToken cancellation)
    {
        ...
        try {
            ...
            try { Volatile.Write(ref slot.Current, await slot.Fetch(cancellation)); }
            catch (OperationCanceledException) when (!cancellation.IsCancellationRequested) { Volatile.Write(ref slot.Current, new(id, Status: "조회 시간 초과 · 재시도 대기")); }
                ...
                Volatile.Write(ref slot.Current, new(id, Status: e is UsageConnectionException or CodexException ? e.Message : "연결 실패 · 설치·로그인·네트워크 확인"));
            }
