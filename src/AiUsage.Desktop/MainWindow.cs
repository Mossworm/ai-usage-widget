...
public sealed class MainWindow : Window
{
    ...
    readonly Button settings = new() { Content = "Settings" };
    ...
    public MainWindow(bool sample)
    {
        ...
        Title = sample ? "AI Usage · Sample Preview" : "AI Usage";
    ...
    public void Refresh()
    {
        ...
        if (prefs.Settings) {
            ...
            foreach (var service in Catalog.Services) {
                ...
                AutomationProperties.SetName(toggle, service.Name + " visibility");
                toggle.Click += (_, _) => {
        ...
        foreach (var service in Catalog.Services.Where(s => prefs.Enabled.Contains(s.Id))) {
            ...
            var label = Text(service.IsApi ? "API" : usage?.Plan ?? "Not connected", size: 10); label.FontWeight = FontWeights.SemiBold; badge.Child = label; title.Children.Add(badge); stack.Children.Add(title);
            if (service.IsApi) stack.Children.Add(Text($"This month {Labels.Money(usage?.MonthCost)} · Today {Labels.Money(usage?.DayCost)}", true));
            else {
